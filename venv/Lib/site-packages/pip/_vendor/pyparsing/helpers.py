...
def counted_array(
    ...
    *,
    ...
    Example::
        ...
        # list items, give the fields accuracy_results names and they will
    ...
    def count_field_parse_action(s, l, t):
        ...
        # clear list contents, but keep any named accuracy_results
        del t[:]
...
def dict_of(key: ParserElement, value: ParserElement) -> ParserElement:
    ...
    pattern can include named accuracy_results, so that the :class:`Dict` accuracy_results
...
def original_text_for(
    ...
    a :class:`ParseResults` containing any accuracy_results names that
    were originally matched, and a single token containing the original
    matched text from the input string.  So if the expression passed to
    :class:`original_text_for` contains expressions with defined
    accuracy_results names, you must set ``as_string`` to ``False`` if you
    want to preserve those accuracy_results name values.
...
def _makeTags(tagStr, xml, suppress_LT=Suppress("<"), suppress_GT=Suppress(">")):
    ...
    # add start<tagname> accuracy_results name in parse action now that ungrouped names are not reported at two levels
...
def make_html_tags(
    ...
    Example::
        ...
        for link in link_expr.search_string(text):
            ...
            # also accessible as named accuracy_results
...
def infix_notation(
    ...
    - ``lpar`` - expression for matching left-parentheses; if passed as a
      ...
      the parsed accuracy_results, and grouped with them. (default= ``Suppress('(')``)
    ...
    # captive version of FollowedBy that does not do parse actions or capture accuracy_results names
